...
export const directorService = {
  ...
  getOverviewStats: async () => {
    try {
      ...
      const [students, teachers, courses, revenue] = await Promise.all([
        ...
        api.get("/api/staffs?role=teacher&limit=1"), // Hoặc endpoint giáo viên của bạn
        api.get("/api/courses?limit=1"),
        api.get("/api/receipts?limit=1000"), // Lấy danh sách để tính tổng tiền
      ]);
      ...
      const getCount = (res) =>
        res.data?.total || res.data?.pagination?.total || res.data?.count || 0;
  ...
  getRevenueChartData: async () => {
    try {
      const response = await api.get("/api/receipts?limit=2000&sort=createdAt");
