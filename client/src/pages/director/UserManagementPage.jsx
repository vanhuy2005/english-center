...
const UserManagementPage = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const response = await apiClient.get(`/api/director/users?${params}`);
  ...
  const handleDeleteUser = async (userId, userName) => {
    ...
    try {
      const response = await apiClient.delete(`/api/director/users/${userId}`);
      if (response.data.success) {
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      const response = await apiClient.post("/api/director/users", submitData);
      console.log("Res: ", response);
      if (response.data.success) {
        toast.success(
          `Tạo tài khoản thành công! MK mặc định: ${
      ...
      >
        <form onSubmit={handleCreateUser} className="space-y-6 p-1">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex flex-wrap gap-1 mt-2">
                    {formData.specialization.map((spec, idx) => (
                      <span
                        key={idx}
                        className="bg-white border border-green-200 text-green-700 px-2 py-0.5 rounded text-xs flex items-center gap-1"
                      >
                        {spec}{" "}
                        <button
                          type="button"
          ...
          <p className="text-xs text-center text-gray-400 mt-2">
            * Mật khẩu mặc định là <strong>123456</strong>. Người dùng cần đổi
            khi đăng nhập.
          </p>
        </form>
      </Modal>
