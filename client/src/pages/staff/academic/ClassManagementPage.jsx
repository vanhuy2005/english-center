...
const ClassManagementPage = () => {
  ...
  const handleCreateClass = async (e) => {
    e.preventDefault();
    if (
      !newClassForm.className ||
      !newClassForm.course ||
      !newClassForm.startDate ||
      !newClassForm.endDate
    ) {
      toast.error("Vui lòng điền các trường bắt buộc (*) và ngày kết thúc");
      return;
    }

    // Validate date ordering
    if (new Date(newClassForm.endDate) <= new Date(newClassForm.startDate)) {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <Badge
        variant={conf.color}
        className="flex items-center gap-1 px-2.5 py-0.5 whitespace-nowrap"
      >
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            <div className="grid grid-cols-3 gap-2">
              <div className="px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg text-center min-w-[80px]">
                <span className="block text-xs text-blue-500 font-semibold uppercase mb-0.5">
                  Tổng
                </span>
              ...
              <div className="px-3 py-2 bg-emerald-50 border border-emerald-100 rounded-lg text-center min-w-[80px]">
                <span className="block text-xs text-emerald-500 font-semibold uppercase mb-0.5">
                  Active
                </span>
              ...
              <div className="px-3 py-2 bg-amber-50 border border-amber-100 rounded-lg text-center min-w-[80px]">
                <span className="block text-xs text-amber-500 font-semibold uppercase mb-0.5">
                  Sắp mở
                </span>
        ...
        {filteredClasses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredClasses.map((cls) => {
              const current =
                cls.currentEnrollment || cls.students?.length || 0;
              ...
              return (
                <Card
                  key={cls._id}
                  className="group border border-gray-200 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 bg-white flex flex-col"
                >
        ...
        >
          <div className="space-y-6 p-1">
            <div className="bg-gray-50 p-3 rounded-lg text-sm border border-gray-100">
              Đang phân công cho lớp:{" "}
              <strong>{selectedClass.className || selectedClass.name}</strong>
            </div>
            <div>
              ...
              <Select
                ...
                options={teachers.map((t) => ({
                  value: t._id,
                  label: `${t.fullName} (${t.staffCode || "TBA"})`,
                }))}
          ...
          <form onSubmit={handleCreateClass} className="space-y-6 p-1">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Input
                ...
                placeholder="VD: Anh Văn Giao Tiếp K12"
              />
              <Input
                label="Mã lớp"
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewClassForm({ ...newClassForm, capacity: e.target.value })
                }
              />
              ...
              <Input
                ...
                placeholder="VD: 2-4-6 (19:00 - 21:00)"
              />
              <Input
                label="Phòng học"
