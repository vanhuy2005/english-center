...
const AssignClassesPage = () => {
  ...
  const fetchPendingRequests = async () => {
    try {
      ...
      // Get both pending and approved requests that haven't been assigned to a class yet
  ...
  const handleAssignSubmit = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        toast.success(
          `Đã xếp lớp thành công cho học viên ${selectedRequest.student?.fullName}`
        );
  ...
  const filteredRequests = requests.filter((r) => {
    ...
    return (
      sName.includes(search) || sCode.includes(search) || cName.includes(search)
    );
  });
        ...
        <Card className="border border-gray-200 shadow-sm">
          <div className="p-4 flex flex-col md:flex-row gap-4 items-center">
            ...
            <div className="flex items-center text-sm text-gray-500 gap-2">
              <Filter size={16} />
              <span>
                Hiển thị: <strong>{filteredRequests.length}</strong> kết quả
              </span>
            </div>
          </div>
        </Card>
          ...
          {filteredRequests.length > 0 ? (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-gray-500 mt-1 pl-6">
                          Mã:{" "}
                          {req.course?.courseCode || req.course?.code || "---"}
                        </div>
                      </td>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400">
              ...
              <p className="text-lg font-medium text-gray-600">
                Không có yêu cầu chờ xếp lớp
              </p>
              <p className="text-sm mt-1">
                Tất cả yêu cầu đã được xử lý hoặc không tìm thấy kết quả.
              </p>
            </div>
          )}
        </Card>
          ...
          <div className="space-y-6 p-1">
            ...
            <div>
              <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2 uppercase tracking-wide">
                <School size={16} className="text-[var(--color-secondary)]" />{" "}
                ...
                {classesMap[selectedRequest.course?._id]?.length > 0 ? (
                    ...
                    return (
                      <div
                        ...
                        className={`
                                   ...
                                   ${
                                     isFull
                                       ? "opacity-60 cursor-not-allowed bg-gray-100 grayscale"
                                       : ""
                                   }
                                `}
                      >
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {isSelected && (
                              <CheckCircle
                                size={16}
                                className="text-[var(--color-secondary)] fill-white"
                              />
                            )}
                          </div>
                          <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-500">
                            <span className="flex items-center gap-1 bg-gray-100 px-1.5 py-0.5 rounded">
                              ...
                              {/* Sử dụng hàm formatSchedule đã sửa */}
                            </span>
                            <span className="flex items-center gap-1">
                              <Users size={10} /> {current}/{max} HV
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="p-8 text-center flex flex-col items-center">
                    ...
                    <p className="text-gray-500 text-sm font-medium">
                      Không tìm thấy lớp học nào cho khóa này.
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                      Vui lòng kiểm tra lại hoặc tạo lớp mới.
                    </p>
                  </div>
                )}
              </div>
