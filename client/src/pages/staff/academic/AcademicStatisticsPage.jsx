...
const AcademicStatisticsPage = () => {
  ...
  const fetchRealData = async () => {
    try {
      ...
      const totalAttendance = studentsList.reduce(
        (sum, s) => sum + (Number(s.attendanceRate) || 0),
        0
      );
      ...
      let attendanceLabels = ["T2", "T3", "T4", "T5", "T6", "T7", "CN"];
      ...
      if (serverStats.attendanceTrend) {
        ...
        if (
          serverStats.attendanceTrend.datasets &&
          serverStats.attendanceTrend.datasets[0]
        ) {
        ...
        <div className="p-5 flex items-start justify-between">
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <h3 className="text-3xl font-extrabold text-[var(--color-primary)]">
                {value}
              </h3>
              {trend && (
                <span
            ...
            <Card className="border border-gray-200 shadow-sm h-full">
              <div className="p-5 border-b border-gray-100 flex justify-between items-center">
                <h3 className="font-bold text-gray-700 flex items-center gap-2 text-sm uppercase tracking-wide">
                  <TrendingUp size={18} className="text-emerald-500" /> Xu hướng
                  chuyên cần (7 ngày qua)
                </h3>
              </div>
              <div className="p-5 h-[320px]">
                <LineChart
                  data={attendanceChartConfig}
                  options={{
          ...
          <div className="lg:col-span-4">
            <Card className="border border-gray-200 shadow-sm h-full">
              <div className="p-5 border-b border-gray-100">
                <h3 className="font-bold text-gray-700 flex items-center gap-2 text-sm uppercase tracking-wide">
                  <PieChart size={18} className="text-blue-500" /> Phân bố kết
                  quả
                </h3>
              </div>
