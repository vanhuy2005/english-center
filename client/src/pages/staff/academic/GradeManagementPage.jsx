...
const GradeManagementPage = () => {
    ...
    try {
      ...
      if (isObjectId) {
        ...
        const response = await api.get(
          `/staff/academic/grades/${selectedClass}`,
          { validateStatus: () => true }
        );
        let gradeRows = [];
        ...
        if (Array.isArray(students) && students.length > 0) {
          ...
          setClassStats({
            ...
            passRate: merged.length
              ? Math.round((passed / merged.length) * 100)
              : 0,
          });
        } else {
          setGrades(gradeRows);
  ...
  const handleGradeChange = (studentId, field, value) => {
    ...
    setGrades((prev) =>
      prev.map((r) => {
        if (r._id === studentId) {
          const mid =
            field === "midterm" ? Number(value) : Number(r.midterm || 0);
          const fin = field === "final" ? Number(value) : Number(r.final || 0);
          const hasMid =
            field === "midterm"
              ? value !== ""
              : r.midterm !== "" && r.midterm !== null;
          const hasFin =
            field === "final"
              ? value !== ""
              : r.final !== "" && r.final !== null;
  ...
  const handleSaveGrade = async (row) => {
    ...
    try {
      ...
      const courseId =
        classRes.data?.data?.course?._id || classRes.data?.data?.course;
      ...
      const payload = {
        ...
        // Send weights matching the UI headers (40% / 60%)
        weights: {
          midterm: 40,
          final: 60,
          attendance: 0,
  ...
  const handleApprove = async (gradeId) => {
    try {
      ...
      setGrades((prev) =>
        prev.map((r) =>
          r.gradeId === gradeId ? { ...r, status: "approved" } : r
        )
      );
    } catch (err) {
        ...
        {selectedClass ? (
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredGrades.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <span
                          className={`text-lg ${getScoreColor(row.average)}`}
                        >
          ...
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-xl border border-dashed border-gray-200">
            ...
            <p className="text-gray-500 font-medium">
              Vui lòng chọn lớp học để bắt đầu nhập điểm
            </p>
          </div>
        )}
      </div>
