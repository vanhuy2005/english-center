...
const AccountantDashboardPage = () => {
  ...
  const loadDashboard = async () => {
    try {
      ...
      // CALL SERVER DASHBOARD AGGREGATION (more accurate & faster)
      const response = await api.get("/staff/accountant/dashboard");

      if (response.data?.success) {
        const svData = response.data.data || {};

        setDashboardData({
          stats: {
            totalRevenue: svData.stats?.totalRevenue || 0,
            monthRevenue: svData.stats?.monthRevenue || 0,
            pendingPayments: svData.stats?.pendingPayments || 0,
            overduePayments: svData.stats?.overduePayments || 0,
          },
          recentTransactions: (svData.recentTransactions || []).map((t) => ({
            _id: t._id,
            transactionCode:
              t.transactionCode || t._id.substring(0, 8).toUpperCase(),
            student: t.student || null,
            class: t.course || t.class || null,
            amount: t.paidAmount || t.amount || 0,
            status: t.status,
            createdAt: t.createdAt,
            paymentMethod: t.paymentMethod,
          })),
          revenueTrend: svData.revenueTrend || dashboardData.revenueTrend,
          paymentStatus: svData.paymentStatus || dashboardData.paymentStatus,
        });
      }
    } catch (error) {
      ...
      <Card className="border border-gray-200 shadow-sm hover:shadow-md transition-all group">
        <div className="p-5 flex items-start justify-between">
          <div>
            ...
            <h3
              className="text-2xl font-extrabold text-[var(--color-primary)] truncate"
              title={value}
            >
              {value}
            </h3>
          ...
          <div
            className={`p-3 rounded-xl border ${styles[color]} group-hover:scale-110 transition-transform`}
          >
            <Icon size={24} />
          </div>
        </div>
  ...
  const QuickActionCard = ({
    ...
    >
      <div
        className={`p-3 rounded-lg ${bgClass} ${colorClass} group-hover:scale-110 transition-transform`}
      >
        <Icon size={24} />
      </div>
      <div className="ml-4 flex-1">
        <h4 className="font-bold text-gray-700 group-hover:text-[var(--color-primary)] transition-colors text-sm">
          {title}
        </h4>
  ...
  const { stats, recentTransactions, revenueTrend, paymentStatus } =
    dashboardData;
    ...
    <div className="min-h-screen bg-gray-50/50 p-6 md:p-8 font-sans text-gray-800">
      <div className="max-w-[1600px] mx-auto space-y-6">
          ...
          <div className="lg:col-span-1">
            <Card className="border border-gray-200 shadow-sm h-full">
              <div className="p-5 border-b border-gray-100">
                <h3 className="font-bold text-gray-700 flex items-center gap-2 text-sm uppercase tracking-wide">
                  <PieIcon size={18} className="text-blue-500" /> Tỷ lệ thanh
                  toán
                </h3>
              </div>
        ...
        <Card className="border border-gray-200 shadow-sm overflow-hidden bg-white">
          <div className="p-5 border-b border-gray-100 flex justify-between items-center">
            <h3 className="font-bold text-gray-700 flex items-center gap-2 text-sm uppercase tracking-wide">
              <Receipt size={18} className="text-[var(--color-secondary)]" />{" "}
              Giao dịch gần đây
            </h3>
            <button
              onClick={() => navigate("/accountant/transactions")}
