...
import { Card, Button, Loading, Input } from "../../../components/common"; // Import path đúng
import {
  BarChart,
  Bar,
  PieChart,
...
const ReceiptStatisticsPage = () => {
  ...
  const handleExport = () => {
    try {
      ...
      (detailedData || []).forEach((m) => {
        lines.push(
          ["ByMethod", m.name || "", m.count || 0, m.value || 0].join(",")
        );
      });
      lines.push([]);
      ...
      (dailyData || []).forEach((d) => {
        lines.push(
          ["Daily", d.fullDate || "", d.receipts || 0, d.amount || 0].join(",")
        );
      });
  ...
  const StatCard = ({
    ...
    // Show numbers formatted, but render strings (e.g. popular method name) as-is.
    const display =
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-6 md:p-8 font-sans text-gray-800">
      <div className="max-w-[1600px] mx-auto space-y-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--color-primary)] flex items-center gap-3">
              <div className="p-2 bg-[var(--color-primary)] rounded-lg shadow-sm">
                ...
                {/* Note: lucide-react BarChart alias issue, using BarChart3 is safer if imported */}
              </div>
              Thống Kê Doanh Thu
            </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="border border-gray-200 shadow-sm p-6">
            <h3 className="font-bold text-gray-800 mb-6 flex items-center gap-2">
              <PieIcon size={20} className="text-blue-500" /> Tỷ lệ phương thức
              thanh toán
            </h3>
            {detailedData.length > 0 ? (
          ...
          <Card className="border border-gray-200 shadow-sm p-6">
            <h3 className="font-bold text-gray-800 mb-6 flex items-center gap-2">
              <TrendingUp size={20} className="text-emerald-500" /> Doanh thu
        ...
        <Card className="border border-gray-200 shadow-sm p-6">
          <h3 className="font-bold text-gray-800 mb-6 flex items-center gap-2">
            <TrendingUp size={20} className="text-blue-600" /> Tổng doanh thu
            tích lũy
          </h3>
          {dailyData.length > 0 ? (
            <div className="h-[300px]">
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="px-2 py-1 rounded-full bg-gray-100 text-xs font-medium text-gray-600 border border-gray-200">
                        {stats.totalAmount > 0
                          ? ((row.value / stats.totalAmount) * 100).toFixed(2)
                          : 0}
                        %
                      </span>
                    </td>
