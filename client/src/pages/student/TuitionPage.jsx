                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      <div className="flex items-start gap-4 flex-1">
                        <div
                          className={`p-3 rounded-full flex-shrink-0 ${statusConfig.bgClass} ${statusConfig.colorClass}`}
                        >
                          {statusConfig.icon}
                        </div>
                        <div className="flex-1">
                          <div className="flex flex-wrap items-center gap-3 mb-1">
                            ...
                            <span
                              className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${statusConfig.bgClass} ${statusConfig.colorClass}`}
                            >
                              {statusConfig.label}
                            </span>
                          </div>
                          <p className="text-sm text-gray-500 mb-2">
                            Mã: {payment.course?.code || "N/A"} • Giao dịch:{" "}
                      ...
                      <div className="hidden md:flex items-center gap-8">
                        ...
                        <div className="min-w-[120px] flex justify-end">
                          ...
                          payment.status === "overdue" ? (
                            <button
                              onClick={() =>
                                navigate(`/tuition/${payment._id}`)
                              }
                              className="px-4 py-2 bg-[var(--color-secondary)] hover:bg-[var(--color-secondary-dark)] text-white text-sm font-medium rounded-lg shadow-sm transition-colors flex items-center gap-2"
                            >
                              <CreditCard size={16} />
                              Thanh toán
                            </button>
                      ...
                      <div className="md:hidden w-full pt-3 border-t border-gray-100 mt-2">
                        {(payment.status === "pending" ||
                          payment.status === "overdue") && (
                          <button
                            onClick={() => navigate(`/tuition/${payment._id}`)}
                            className="w-full py-2.5 bg-[var(--color-secondary)] text-white text-sm font-bold rounded-lg"
                          >
                            Thanh toán ngay
                          </button>
                        )}
